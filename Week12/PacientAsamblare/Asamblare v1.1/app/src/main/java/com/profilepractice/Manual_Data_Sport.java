...
import utility.ManualDSportAsync;


public class Manual_Data_Sport extends AppCompatActivity implements AsyncResponse {
    ...
    public void Verify(View view) {
        ...
        if (Value1.length() == 0 || Value2.length() == 0) {
            ...
            try {
                ...
                ManualDSportAsync asyncTask = new ManualDSportAsync();
                asyncTask.delegate = this;
                asyncTask.execute(user, Value2, Value1);
    ...
    public void processFinish(Boolean output) throws JSONException {
        ...
        if(output==true)
        {
            Intent intent = new Intent(Manual_Data_Sport.this, Manual_Data_Introduction.class);
            intent.putExtra("EXTRA_USER", user);
            Toast.makeText(Manual_Data_Sport.this,"Sport file sent!",Toast.LENGTH_SHORT).show();
            startActivity(intent);

        }
        else
        {
            Toast.makeText(Manual_Data_Sport.this,"Sport file not sent!", Toast.LENGTH_SHORT).show();
        }
